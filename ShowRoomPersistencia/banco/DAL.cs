...
using ShowRoomPersistencia.classes;
using MySql.Data.MySqlClient;

namespace ShowRoomPersistencia.banco
{
    ...
    public class DAL
    {
        ...
        /// </summary>
        /// <code>
        ///    Leitor l = DAL.GetLeitorFromSQL("select cd_funcionario, nome from funcionario order by 1 desc");
        ///    while (!l.Eof)
        ///    {
        ...
        /// </summary>
        /// <code>
        ///    DataTable dt = DAL.ListarFromSQL("select cd_funcionario, nome from funcionario order by 1 desc");
        ///    foreach (DataRow dr in dt.Rows)
        ///        System.Console.WriteLine(dr["cd_funcionario"] + "|" + dr["nome"]);
        /// </code>
        /// <param name="sql">Código sql para executar na base</param>
        ...
        protected static string GetSqlSelect(object data, string filtro = "", string ordem = "", string group = "", int limite = 0)
        {
            ...
            sql.Append(" from ");
            sql.Append(Montador.GetTableName(data) + " a ");
            // temos where?
        ...
        public static string GetSqlInsertUpdate(string tableName, List<Campo> campos)
        {
            ...
            foreach (Campo campo in campos)
            {
                if (!campo.IsOnlyUpdate)
                {
                    if (!campo.IsOnlySelect)
                    {
                        sqlCampos.Append(string.Format("{0},", campo.Nome));
        ...
        ///    while (!l.Eof)
        ///    {
        ///        System.Console.WriteLine(l.GetString("cd_empresa") + "-" + l.GetString("nm_empresa") + "=" + l.GetString("cd_funcionario") + "-" + l.GetString("nome"));
        ///        l.Next();
        ///    }
        /// </code>
        ...
        ///     
        ///     foreach (DataRow dr in dt.Rows)
        ///         System.Console.WriteLine(dr["cd_empresa"] + "-" + dr["nm_empresa"] + "=" + dr["cd_funcionario"] + "-" + dr["nome"]);
        /// </code>
        ...
        /// <returns>Retorna um valor</returns>
        /// <code>
        ///    string nome = DAL.GetString("select nome from funcionario where id=10");
